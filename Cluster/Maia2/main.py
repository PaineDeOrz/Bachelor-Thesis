...
def train_chunks(cfg, data, model, optimizer, all_moves_dict, criterion_maia, criterion_side_info, criterion_value):
    ...
    #print(f"DEBUG: Dataset len BEFORE: {len(dataset_train.data)}")
    total_positions = len(dataset_train.data)
    full_batches = total_positions // cfg.batch_size
    dataset_train.data = dataset_train.data[:full_batches * cfg.batch_size]
    #print(f"DEBUG: {total_positions} -> truncate to {full_batches * cfg.batch_size})")
    #print(f"DEBUG: Dataset len AFTER:  {len(dataset_train.data)}")
    ...
    for boards, labels, elos_self, elos_oppo, legal_moves, side_info, wdl in dataloader_train:
        ...
        #print(f"BATCH {step}: boards.shape={boards.shape}, batch_size={boards.size(0)}")
        #print(f"EXPECTED: {cfg.batch_size}, GOT: {boards.size(0)}")
        assert boards.size(0) == cfg.batch_size, f"RAGGED BATCH! {boards.size(0)} != {cfg.batch_size}"
        ...
        #print(f"BEFORE MODEL: boards.numel()={boards.numel()}")
        #print(f"EXPECTED numel: {cfg.batch_size * cfg.input_channels * 64}")
        expected_numel = cfg.batch_size * cfg.input_channels * 64
        assert boards.numel() == expected_numel, f"TENSOR SIZE WRONG: {boards.numel()} != {expected_numel}"
